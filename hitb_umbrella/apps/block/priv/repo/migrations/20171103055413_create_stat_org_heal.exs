defmodule Block.Stat.Repo.Migrations.CreateStatOrgHeal do
  use Ecto.Migration

  def change do
    create table(:stat_org_heal) do
      add :time, :string #时间
      add :org, :string #机构
      add :true_org, :string #机构
      add :num_sum, :integer #病历数
      add :death_rate, :float #死亡率
      add :day_avg, :float #平均住院日
      add :fee_avg, :float #平均费用
      add :heal_fee_avg, :float #平均医保费用
      add :self_fee_avg, :float #平均个人医保费用
      add :fee_gl, :float #管理费用
      add :fee_hl, :float #护理费用
      add :fee_yj, :float #医技费用 无
      add :fee_yl, :float  #医疗费用
      add :fee_yp, :float #药品费用
      add :pay_rate, :float #医保支付率
      add :selfpay_rate, :float #个人支付率
      add :time_type, :string #时间维度
      add :org_type, :string #机构维度
      add :int_time, :integer #时间排序
      add :previous_hash, :string
      add :hash, :string
      add :datetime, :string
      timestamps()
    end
    create unique_index(:stat_org_heal, [:hash])
  end
end
